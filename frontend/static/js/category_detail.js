...
// 返回上一页
function goBack() {
    // 如果来自年度概览，返回到支出统计页面（带年份）
    if (fromYear) {
        window.location.href = `/category-stats?start=${startDate}&end=${endDate}&year=${fromYear}`;
    } else {
        window.history.back();
    }
}
...
async function loadCategoryDetail() {
    ...
    try {
        // 获取分类详情统计数据
        const statsUrl = `${API_BASE}/category-detail/${categoryId}?start_date=${startDate}&end_date=${endDate}`;
