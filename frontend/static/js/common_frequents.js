...
// ==================== 加载常用收藏记事 ====================
async function loadCommonFrequents() {
    try {
        const response = await fetch(`${API_BASE}/reflections/common-frequents?page=${currentPage}&page_size=${pageSize}`, getAuthOptions());
        ...
        // 渲染记事列表
        const list = document.getElementById('common-frequents-list');
        ...
        <div class="item-card reflection-card frequent" data-id="${item.id}">
            ...
            <div class="reflection-card-footer">
                ...
                <div class="item-actions" onclick="event.stopPropagation()">
                    <button class="btn-icon btn-frequent active" onclick="toggleCommonFrequent(${item.id})" title="取消收藏">
                        <span>💝</span>
                    </button>
...
// ==================== 分页功能 ====================
function changeCommonFrequentsPage(delta) {
...
function updatePagination() {
    document.getElementById('common-frequents-current-page').textContent = currentPage;
    document.getElementById('common-frequents-total-pages').textContent = totalPages;
    document.getElementById('common-frequents-total-items').textContent = totalItems;

    const prevBtn = document.getElementById('common-frequents-prev-btn');
    const nextBtn = document.getElementById('common-frequents-next-btn');

    prevBtn.disabled = currentPage <= 1;
    nextBtn.disabled = currentPage >= totalPages;

    // 如果没有数据，隐藏分页
    const paginationContainer = document.getElementById('common-frequents-pagination');
    if (totalItems === 0) {
...
async function saveItem() {
    ...
    const isCommonFrequent = document.getElementById('isCommonFrequent').checked;
...
// ==================== 切换收藏状态 ====================
async function toggleCommonFrequent(id) {
    try {
        // 直接获取该记事的详情
        const response = await fetch(`${API_BASE}/reflections/${id}`, getAuthOptions());
